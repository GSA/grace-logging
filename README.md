...
|[AU-6(1)](https://nvd.nist.gov/800-53/Rev4/control/AU-6#enhancement-1) || AWS CloudTrail logs are forwarded to the GSA Enterprise Logging Platform (SecOps). The GSA Security Operations team  monitors and evaluates these logs for potential misuse of information system accounts as part of their daily Security Operations Center(SOC) process. An additional process for monitoring and alerting will need to followed.  | CSP: AWS (CloudTrail, S3) | Application, Database, Flow Logs, other AWS logs |
...
||d| The GSA Security Operations team protects information obtained from AWS CloudTrail logs from unauthorized access, modification, and deletion on Enterprise Logging Platform.  Logs are also stored in GRACE S3 buckets and Amazon Glacier.  | CSP: AWS (CloudTrail, S3) | Application, Database, Flow Logs, other AWS logs |
